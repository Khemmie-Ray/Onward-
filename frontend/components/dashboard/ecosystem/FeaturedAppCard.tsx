...
import { ArrowRight, Coins, Globe } from "lucide-react";
...
export function FeaturedAppCard({ app }: { app: EcosystemApp }) {
  ...
  return (
    <Link
      ...
      className={`group relative block overflow-hidden rounded-[20px] p-6 transition-transform hover:-translate-y-1 ${t.bg} shadow-[0_8px_24px_rgba(31,58,110,0.10)]`}
    >
      <div
        aria-hidden
      ...
      <div className="relative">
        <div className="flex items-start justify-between mb-3">
          <div className={`flex h-12 w-12 items-center justify-center rounded-[14px] ${t.iconBg}`}>
            <Globe size={22} strokeWidth={2.2} className={t.iconColor} />
          </div>
          <span className={`text-[9px] font-bold uppercase tracking-[0.12em] ${t.accent}`}>
            {app.category}
          </span>
        </div>
        ...
        <p className="text-[13px] font-medium text-fg-soft mb-3">{app.tagline}</p>
        <p className="text-[12.5px] leading-[1.55] text-fg-soft mb-4">{app.description}</p>
